...
const char gRaceMotionViewerMotionNumberFormat[] = "MOTION NO %3.3i";
...
void drawRaceMotionViewerMotionNumber(s32 arg0) {
    char buf[0x64];

    sprintf(buf, gRaceMotionViewerMotionNumberFormat,
...
void updateRaceMotionViewer(void) {
    ...
    if (gRaceMotionViewerState.initialized == 0) {
        ...
        if (buttons & 0x2000) {
            if (gRaceMotionViewerState.modelFlags != 0) {
                gRaceMotionViewerState.modelFlags = 0;
            } else {
                gRaceMotionViewerState.modelFlags = 0x400;
            }
        }
    ...
    addRenderCallback(&gMenuForegroundRenderCallbackList, drawRaceMotionViewerMotionNumber, NULL);
}
