...
// interpolateRaceMotionJointAnimationFrame best match: 98.123% (nonmatchings/interpolateRaceMotionJointAnimationFrame-3885303446860889946/base_11.c)
...
loop_rot:
        rootSample->x = frameData[0] << 14;
        rootYShift = 14;
        rootSample->y = frameData[1] << rootYShift;
        rootSample->z = frameData[2] << 14;
        i++;
        rootSample++;
        frameData += 3;
...
loop_pos:
        ...
        jointSample++;
        decodeLimit = RACE_MOTION_INTERPOLATED_JOINT_COUNT * sizeof(RaceMotionRotation);
        if ((jointOffset ^ 0) < decodeLimit) {
            goto loop_pos;
        }
...
loop_skip:
    ...
    jointSample = gRaceMotionJointFrameBufferReload;
    i = 0;
    jointCursor = state;
loop_interp:
    x = jointSample->x;
    ...
    modelJoint = state->animation.motion.jointX;
    state->animation.motion.rootMotion.position.x =
        (((gRaceMotionRotationFrameBufferReload[6] - rootData[0]) * frameTimer) / frameTimerReset) + rootData[0];
    state->animation.motion.rootMotion.position.y =
        (rootRotationZ = (((rootData[7] - rootData[1]) * frameTimer) / frameTimerReset) + rootData[1]);
    state->animation.motion.rootMotion.position.z =
        (((rootData[8] - rootData[2]) * frameTimer) / frameTimerReset) + rootData[2];
    state->animation.motion.rootMotion.rotation.x =
        (((rootData[9] - rootData[3]) * frameTimer) / frameTimerReset) + rootData[3];
    state->animation.motion.rootMotion.rotation.y =
        (((gRaceMotionRotationFrameBufferReload[10] - gRaceMotionRotationFrameBufferReload[4]) * frameTimer) /
             frameTimerReset) +
        gRaceMotionRotationFrameBufferReload[4];
    rootRotationZ = (((rootData[11] - rootData[5]) * frameTimer) / frameTimerReset) + rootData[5];
