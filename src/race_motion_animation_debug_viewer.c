...
const char gRaceMotionAnimationDebugViewerNumberFormat[] = "MOTION NO %3.3i";
...
void drawRaceMotionAnimationDebugViewerNumber(s32 arg0) {
    char buf[0x64];

    sprintf(buf, gRaceMotionAnimationDebugViewerNumberFormat, gRaceMotionAnimationDebugViewerState.animationIndex);
...
void updateRaceMotionAnimationDebugViewer(void) {
    ...
    if (gRaceMotionAnimationDebugViewerState.initialized == 0) {
        gRaceMotionAnimationDebugViewerState.initialized = 1;
        ...
        if ((buttons & 8) && (gRaceMotionAnimationDebugViewerState.animationIndex != 0)) {
            gRaceMotionAnimationDebugViewerState.animationIndex--;
            gRaceMotionAnimationDebugViewerState.initialized = 0;
        }
        if (buttons & 4) {
            gRaceMotionAnimationDebugViewerState.initialized = 0;
            gRaceMotionAnimationDebugViewerState.animationIndex++;
    ...
    addRenderCallback(&gMenuForegroundRenderCallbackList, drawRaceMotionAnimationDebugViewerNumber, NULL);
}
