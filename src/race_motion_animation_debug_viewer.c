...
void drawRaceMotionAnimationDebugViewerMotionNumber(s32 arg0) {
    char buf[0x64];

    sprintf(buf, gRaceMotionAnimationDebugViewerMotionNumberFormat,
            gRaceMotionAnimationDebugViewerState.motionIndex);
...
void updateRaceMotionAnimationDebugViewer(void) {
    ...
    if (gRaceMotionAnimationDebugViewerState.initialized == 0) {
        ...
        if (buttons & 0x2000) {
            if (gRaceMotionAnimationDebugViewerState.renderMode != 0) {
                gRaceMotionAnimationDebugViewerState.renderMode = 0;
            } else {
                gRaceMotionAnimationDebugViewerState.renderMode = 0x400;
            }
        }
        if ((buttons & 8) && (gRaceMotionAnimationDebugViewerState.motionIndex != 0)) {
            gRaceMotionAnimationDebugViewerState.motionIndex--;
