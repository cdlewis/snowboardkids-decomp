...
loop_24:
    ...
    gCharacterSelectCourseCursorState.listCursorState = 0;
    gCharacterSelectCourseCursorState.submenuState = 0;
    gCharacterSelectCourseCursorState.previewFrameState = 0;
    gCharacterSelectCourseCursorState.listCursorSpriteIndex = 0;
    gCharacterSelectCourseCursorState.listCursorTimer = 0;
...
void initCharacterSelectCourseMenuFromPlayerSelect(void) {
    ...
    gCharacterSelectCourseCursorState.listCursorSpriteIndex = 0;
    gCharacterSelectCourseCursorState.listCursorTimer = 0;
    gCharacterSelectCourseCursorState.submenuTimer = 0 * 0;
    updateCallbackTasks();
}
// clang-format on
    ...
    if (gCurrentGameTask->callbackData0 != 0) {
        ...
        if (gRacePlayers[0].menuState == 0) {
            if (gMenuSelectionConfirmTimer == 0) {
                ...
                if (gCharacterSelectCourseCursorState.listCursorState == 1) {
 ...
 upInput = input & (STICK_UP | U_JPAD); if ((upInput == 0) && ((input & (STICK_DOWN | D_JPAD)) == 0)) { *repeatTimerPtr = 0; } pressedInput = *pressedInputPtr; break; } repeatTimer = *repeatTimerPtr; if ((pressedInput & (STICK_UP | U_JPAD)) || (((upInput != 0) && (((s32)repeatTimer) >= 0xB)) && (((((s32)repeatTimer)) % 3) == 0))) { if (!repeatTimer) { repeatTimer += 1; } if (selection > 0) { gRaceCourseIndex.signedValue = selection - 1; selection = gRaceCourseIndex.signedValue; } *repeatTimerPtr = repeatTimer; } else if ((pressedInput & (STICK_DOWN | D_JPAD)) || ((((*repeatTimerPtr = repeatTimer, input & (STICK_DOWN | D_JPAD))) && ((*repeatTimerPtr = (upInput = repeatTimer), ((s32)repeatTimer) >= 0xB))) && ((*repeatTimerPtr = repeatTimer, ((((s32)repeatTimer)) % 3) == 0)))) { *repeatTimerPtr = repeatTimer; if (repeatTimer == 0) { *repeatTimerPtr = repeatTimer + 1; } if ((*gCharacterSelectActiveCourseOptions)[selection] != (-1)) { gRaceCourseIndex.signedValue = selection + 1;
                    ...
                    if ((((((((((input & START_BUTTON) & 0xFFFFu) & 0xFFFFu) & 0xFFFFu) & 0xFFFFu) & 0xFFFFu) &
                        ...
                        if ((*gCharacterSelectActiveCourseOptions)[gRaceCourseIndex.signedValue] != -1) {
                            ...
                            gCharacterSelectCourseCursorState.listCursorState = 2;
                            gCharacterSelectCourseCursorState.listCursorSpriteIndex = spriteIndex;
                        } else {
                            gCharacterSelectCourseCursorState.listCursorState = 2;
                            gCharacterSelectCourseCursorState.listCursorSpriteIndex = spriteIndex;
        ...
        if (gCharacterSelectCourseCursorState.submenuState >= 2) {
...
void updateCharacterSelectCourseSubmenu(void) {
    ...
    if (state < 3) {
        switch (gCharacterSelectCourseCursorState.submenuState) {
            case 2:
                ...
                if (input & B_BUTTON) {
                    ...
                    gCharacterSelectCourseCursorState.submenuState = 3;
            ...
            case 3:
                if (gMenuChoicePromptState[0] >= 3) {
                    ...
                    if ((input & A_BUTTON) || (input & START_BUTTON)) {
                        ...
                        gCharacterSelectCourseCursorState.submenuState = 4;
                ...
                break;
            case 4:
                if (gCharacterSelectCourseCursorState.submenuTimer == 4) {
        ...
        if (state == 6) {
            ...
            gMenuSelectionConfirmTimer = 0;
            setCurrentGameTaskCallback(updateCharacterSelectCourseMenu, 0);
            gCharacterSelectCourseCursorState.listCursorState = 1;
            gCharacterSelectCourseCursorState.listCursorSpriteIndex = 0x100;
            gCharacterSelectCourseCursorState.listCursorTimer = 0;
            state = *menuState;
        }
        if (state == 8) {
