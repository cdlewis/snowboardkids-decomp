...
// Matched by inspectredc via decomp.me scratch LH2aY.
void drawScaledAssetTableSpriteWithExplicitPalette(s16 x, s16 y, AssetTable *asset, u16 entryIndex,
...
void runRenderCallbacks(RenderCallbackNode **arg0) {
    ...
    if (s0 != NULL) {
    loop:
        if ((u32)(((u8 *)gRegionAllocPtr - (u8 *)gCurrentFrameRenderData) - sizeof(FrameRenderData)) < 0x14181U) {
            s0->callback(s0->arg);
            s0 = s0->next;
            if (s0 != NULL) {
                goto loop;
