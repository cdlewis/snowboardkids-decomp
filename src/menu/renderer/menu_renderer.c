...
// drawMenuSpriteWithPaletteScale best logical match: 98.532%
// (nonmatchings/drawMenuSpriteWithPaletteScale-8101714008744796594/base_27.c)
// base_52.c scores 98.837% but relies on redundant channel masks and is retained only as a compiler diagnostic.
...
// drawMenuSpriteFixedScale best match: 90.012%
// (nonmatchings/drawMenuSpriteFixedScale-2641000770066553983/base_32.c)
    ...
    if (x0 < maxX) {
        ...
        if ((i < maxY) && (viewHalfWidth = x1 >= minX)) {
            ...
            if (y1 >= minY) {
                ...
                dstPalette = scaledPalette;
paletteLoop:
                *dstPalette = (paletteColor = *(u16 *)&((MenuGlyphPalette *)srcPalette)->bytes[i] & 0xFFFFFFFFFFFFFFFF);
                i += sizeof(u16);
                do {
                    color = paletteColor & 0xFFFF;
                    do {
