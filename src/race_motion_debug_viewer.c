...
const char gRaceMotionDebugViewerAnimationNumberFormat[] = "MOTION NO %3.3i";
...
void drawRaceMotionDebugViewerAnimationNumber(s32 arg0) {
    char buf[0x64];

    sprintf(buf, gRaceMotionDebugViewerAnimationNumberFormat, gRaceMotionDebugViewerState.animationIndex);
...
void updateRaceMotionDebugViewer(void) {
    ...
    if (gRaceMotionDebugViewerState.initialized == 0) {
        ...
        setRaceMotionAnimation((RaceMotionState *)&gRaceMotionDebugViewerState,
                               gRaceMotionDebugViewerState.animationIndex);
        ...
        if ((buttons & 8) && (gRaceMotionDebugViewerState.animationIndex != 0)) {
            gRaceMotionDebugViewerState.animationIndex--;
    ...
    addRenderCallback(&gMenuForegroundRenderCallbackList, drawRaceMotionDebugViewerAnimationNumber, NULL);
}
