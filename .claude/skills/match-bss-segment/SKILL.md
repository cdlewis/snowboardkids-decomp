...
description: Match a segment's bss data by replacing extern declarations with the appropriate definitions. Use when asked to match, add, split, or fix a .bss segment, especially after checksum failures from bss symbol layout, shifted VRAM labels, queue/stack buffers, or unresolved bss externs.
---
...
1. Identify the target range.
   ...
   - Check `snowboardkids.yaml` for the current unsplit `bss` segment covering that range.

2. Split `snowboardkids.yaml`.
...
```bash
mips-linux-gnu-nm -n build/snowboardkids.elf | rg "symbolA|symbolB|target_file"
```
...
7. Finish only after full verification.
   - `./tools/build-and-verify.sh` must finish with `build/snowboardkids.z64: OK`.
