...
## func_80063E70 (race_ui_effects)

- For particle array update callbacks that index a 12-byte record array across
  calls, adding `register` to both the actor alias and loop counter can be
  enough to make IDO allocate the actor in `$s0`, the generated byte offset in
  `$s1`, and the counter in `$s2`. The same source without `register` matched
  behavior but swapped `$s0`/`$s1`.

## func_800668EC (race_overlay_effects)

- Overlay model table entries are 0x14 bytes: `s16 modelIndex`, padding,
